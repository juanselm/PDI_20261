...
4. El analisis generara:
   ...
   - video_procesado.mp4        (video con trayectoria y velocidad)
...
# === VIDEO PROCESADO ===
print("[*] Generando video procesado...")
...
for i, frame in enumerate(frames):
    ...
    diff = cv2.absdiff(frame, background)
    gray = cv2.cvtColor(diff, cv2.COLOR_BGR2GRAY)
    _, mask_frame = cv2.threshold(gray, THRESHOLD, 255, cv2.THRESH_BINARY)
    ...
    cv2.putText(frame_out, 'A', (PUNTO_A_X - 15, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2)
    cv2.putText(frame_out, 'B', (PUNTO_B_X + 5, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2)
    ...
    cv2.line(frame_out, (bar_x + bar_length_px, bar_y - 5), (bar_x + bar_length_px, bar_y + 5), (255, 255, 255), 2)
    cv2.putText(frame_out, f'{{bar_length_m:.0f}} m', (bar_x + bar_length_px // 2 - 20, bar_y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
    ...
    cv2.putText(frame_out, f't = {{t:.2f}} s   Frame {{i}}/{{total_frames}}', (12, 28), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)

    if cx_det is not None:
        pos_m = cx_det * SCALE
        cv2.putText(frame_out, f'Pos: {{pos_m:.2f}} m  ({{cx_det}} px)', (12, 52), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 1)
        cv2.putText(frame_out, f'Vel: {{vx_ms:.2f}} m/s  ({{vx_kmh:.1f}} km/h)', (12, 76), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)
        cv2.putText(frame_out, f'Escala: {{SCALE:.4f}} m/px', (12, 100), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (200, 200, 200), 1)
    else:
        cv2.putText(frame_out, 'Sin deteccion', (12, 52), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (100, 100, 100), 1)

    out_video.write(frame_out)

out_video.release()
print(f"Video procesado guardado: {{VIDEO_PROCESADO_PATH}}")
...
"""
            ...
            if result.returncode == 0:
                ...
                self._log("  - video_procesado.mp4")
                messagebox.showinfo("Exito", "Análisis completado. Revisa los archivos PNG y el video generados.")
            else:
